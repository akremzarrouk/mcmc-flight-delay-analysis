...
[![CI](https://github.com/akremzarrouk/mcmc-flight-delay-analysis/actions/workflows/ci.yml/badge.svg)](https://github.com/akremzarrouk/mcmc-flight-delay-analysis/actions/workflows/ci.yml)
...
├── src/                                  # Core logic backing both Python notebooks
│   ├── mcmc.py                           #   sampler + Gelman-Rubin diagnostic
│   └── flight_pipeline.py                #   load/clean/feature-engineer/analyze/model
├── tests/                                # pytest unit tests + a pipeline smoke test
├── scripts/generate_sample_data.py       # (re)generates data/sample/
├── images/                               # Output plots embedded above
├── requirements.txt                      # Python dependencies
├── requirements-dev.txt                  # + testing/CI dependencies
├── .github/workflows/ci.yml              # runs tests + executes the MCMC notebook
├── LICENSE                               # MIT
└── data/
    ├── sample/                           # tiny synthetic dataset, committed (see below)
    └── ...                               # full flight data, not committed (see below)
```

The notebooks are the narrative (methodology, plots, findings); `src/` holds the
same logic as tested, importable modules so it isn't only verified by eyeballing
notebook output.
...
**Just want to try the pipeline without the download?** `data/sample/` has a small
synthetic dataset (~4,000 rows, same schema, committed to the repo) generated by
[`scripts/generate_sample_data.py`](scripts/generate_sample_data.py):

```python
from src.flight_pipeline import run_analysis
run_analysis(
    flight_files=["data/sample/flights.csv"],
    plane_file="data/sample/plane-data.csv",
    airport_file="data/sample/airports.csv",
    carrier_file="data/sample/carriers.csv",
)
```
...
Unit tests cover the MCMC sampler/`R-hat` diagnostic (`tests/test_mcmc.py`) and the
flight-pipeline cleaning/feature-engineering functions (`tests/test_flight_pipeline.py`)
against small synthetic DataFrames. `tests/test_pipeline_smoke.py` runs the full
`run_analysis()` pipeline end-to-end against `data/sample/`, so a broken pipeline
fails CI without needing the real 3.3 GB dataset. CI (`.github/workflows/ci.yml`)
runs this suite on every push/PR and also executes `mcmc_metropolis_hastings.ipynb`
via `nbconvert` to catch notebook rot.
